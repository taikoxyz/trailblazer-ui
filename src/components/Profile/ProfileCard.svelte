<script lang="ts">
  ...
  function handleSetDomain(selectedDomain: string) {
        ...
        {#if profile?.dotTaiko && profile?.zns}
          <details bind:this={dropdown} class="dropdown dropdown-end ml-1 flex">
            ...
            <ul class="menu dropdown-content bg-base-100 rounded-box z-[1] w-52 p-2 shadow">
              <li><button on:click={() => handleSetDomain(profile.dotTaiko || '')}>{profile.dotTaiko}</button></li>
              <li><button on:click={() => handleSetDomain(profile.zns || '')}>{profile.zns}</button></li>
            </ul>
          </details>
        {/if}
      </div>
