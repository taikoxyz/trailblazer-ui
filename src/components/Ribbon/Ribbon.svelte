...
<div bind:this={outerRibbon} class="w-screen bg-pink-400 overflow-hidden h-6 flex">
  <div bind:this={ribbonGroup} class="w-full flex gap-[15px] animate-move">
    {#each Array.from({ length: count + 2 }, (_, i) => i) as _}
      ...
      <div bind:this={innerRibbon} class="min-w-fit flex flex-nowrap break-keep overflow-visible">
        <div class="body-bold mr-[10px] text-secondary-warm-yellow">WEEK 1</div>
        <div class="body-bold">NOW LIVE</div>
      </div>
    {/each}
  </div>
</div>
