...
export const load = async () => {
  ...
  if (browser) {
    try {
      const page = await userLeaderboardService.getUserLeaderboardData(pageInfo, 3);
      currentUserLeaderboardUserEntry.set(
        await userLeaderboardService.getUserLeaderboardDataForAddress(3, getConnectedAddress()),
      );
      if (page) {
