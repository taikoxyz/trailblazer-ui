...
export const load = async () => {
  ...
  if (browser) {
    try {
      const page = await liquidityCompetitionService.getLiquidityCompetitionLeaderboard(pageInfo, 3);
      currentLiquidityCompetitionLeaderboardUserEntry.set(
        await liquidityCompetitionService.getLiquidityCompetitionDataForAddress(3, getConnectedAddress()),
      );
      if (page) {
