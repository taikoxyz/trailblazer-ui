...
export class ProfileS2 {
  ...
  static async getProfile(address?: string) {
    ...
    if (address) {
      const { data } = await axios.get(`${baseApiUrl}/s2/user/rank`, { params: { address }, ...globalAxiosConfig });
  ...
  static async getUserPointsHistory(address?: string, page?: number) {
    ...
    if (address) {
      const response = await axios.get(`${baseApiUrl}/s2/user/history`, {
        params: { address, page },
      });
